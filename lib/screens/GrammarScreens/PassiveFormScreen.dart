...
class _PassiveScreenState extends State<PassiveScreen> {
  ...
  Future<void> _fetchQuestionsFromFirestore() async {
    try {
      ...
      final snapshot = await firestore.collection('Active-Passive').doc('Questions').get();
  ...
  void submitAnswer() {
    setState(() {
      ...
      String correctAnswer = questions[currentQuestionIndex]['Passive'];
      ...
      if (userAnswer == correctAnswer) {
        ...
        usersCollection.doc(userId).get().then((docSnapshot) {
          if (docSnapshot.exists) {
            ...
            final questionsCompleted = userData['Questions Completed']['Passive'] ?? 0;

            // Compare with the current question number and update if necessary
            if (questionsCompleted <= currentQuestionIndex) {
              usersCollection.doc(userId).update({
                'Questions Completed.Passive': currentQuestionIndex+1,
              });
            }
          }
        });
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 10),
                  Text(
                    "Give the Passive form of the following sentence:",
                    style: TextStyle(
                      fontSize: 16,
                      color: Colors.white,
                    ),
                  ...
                  isLoading // Display loading circle if isLoading is true
                    ...
                    child: Text(
                      questions[currentQuestionIndex]['Active'],
                      style: TextStyle(

...
class NegativeFormScreen extends StatefulWidget {
  @override
  _NegativeFormScreenState createState() => _NegativeFormScreenState();
}

class _NegativeFormScreenState extends State<NegativeFormScreen> {
  ...
  Future<void> _fetchQuestionsFromFirestore() async {
    try {
      ...
      final snapshot = await firestore.collection('Negative Form').doc('Questions').get();
      ...
      if (correctAnswers.contains(userAnswer)) {
        ...
        usersCollection.doc(userId).get().then((docSnapshot) {
          if (docSnapshot.exists) {
            ...
            final questionsCompleted = userData['Questions Completed']['Negative Form'] ?? 0;

            if (questionsCompleted <= currentQuestionIndex) {
              usersCollection.doc(userId).update({
                'Questions Completed.Negative Form': currentQuestionIndex + 1,
              });
            }
          }
        });

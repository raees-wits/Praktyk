...
class IndirectSpeechScreen extends StatefulWidget {
  @override
  _IndirectSpeechScreenState createState() => _IndirectSpeechScreenState();
}

class _IndirectSpeechScreenState extends State<IndirectSpeechScreen> {
      ...
      if (correctAnswers.contains(userAnswer)) {
        ...
        usersCollection.doc(userId).get().then((docSnapshot) {
          if (docSnapshot.exists) {
            ...
            final questionsCompleted = userData['Questions Completed']['Indirect Speech'] ?? 0;

            if (questionsCompleted <= currentQuestionIndex) {
              usersCollection.doc(userId).update({
                'Questions Completed.Indirect Speech': currentQuestionIndex + 1,
              });
            }
          }
        });
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 10),
                  Text(
                    "Write the following question in indirect speech:",
                    style: TextStyle(
                      fontSize: 16,
                      color: Colors.white,
                    ),
